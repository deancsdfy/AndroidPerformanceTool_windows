...
def get_current_packagename():
    ...
    print('1'+package.decode())
    #用-1，是因为部分机型上，还会返回一些系统进程和包，比如小米8
    # print(pattern.findall(package.decode())[0].split('/')[-1])
    # return pattern.findall(package.decode())[0].split('/')[-1]
...
def get_current_activity():
    ...
    #package = shell("dumpsys activity | findstr  mFocusedActivity").stdout.read()
    # print(shell('adb devices').stdout.read())
    # package = shell('dumpsys activity top').stdout.read()
    print(shell('dumpsys activity top').stdout.read())
    # print(pattern.findall(package.decode()).split('/')[-1])
