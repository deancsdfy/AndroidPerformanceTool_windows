...
class ClientPerformanceMoniter(object):
    ...
    def StartMonitor(self):
        ...
        while 1:
            ...
            worksheet.write((count+1)%10,0, label = str(now))
            worksheet.write((count+1)%10,1, label = round(float(self._CurrentCpuUseRate),2))
            worksheet.write((count+1)%10,2, label = round(float(self._CurrentMemUse),1))
