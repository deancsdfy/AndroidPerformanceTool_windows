...
class ClientPerformanceMoniter(object):
    ...
    def StartMonitor(self):
        ...
        while 1:
            ...
            # 写入excel
            # 参数对应 行, 列, 值
            worksheet.write(count+1,1, label = str(time.time()))
            worksheet.write(count+1,2, label = str(self._CurrentCpuUseRate))
            worksheet.write(count+1,3, label = str(self._CurrentMemUse))
            # 保存
