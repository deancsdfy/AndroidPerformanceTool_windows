...
class ClientPerformanceMoniter(object):
    ...
    def _GetPerformanceDataThread(self):
        while True:
           # cpuRate,mem = info.top()
            ...
            mem = info.getMemInfo()
            print("---main function cpu is {}% mem is {}M---".format(cpuRate,mem))
    ...
    def StartMonitor(self):
        ...
        while 1:
            ...
            worksheet.write((count+1)%60,0, label = str(now))
            worksheet.write((count+1)%60,1, label = round(float(self._CurrentCpuUseRate),2))
            worksheet.write((count+1)%60,2, label = round(float(self._CurrentMemUse),1))
