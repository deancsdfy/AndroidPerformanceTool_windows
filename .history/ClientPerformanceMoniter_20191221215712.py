...
class ClientPerformanceMoniter(object):
    ...
    def StartMonitor(self):
        ...
        while 1:
            ...
            # 创建一个worksheet
            slef.worksheet = self.workbook.add_sheet('My Worksheet')

            # 写入excel
            # 参数对应 行, 列, 值
            self.worksheet.write(i+1,1, label = str(time.time()))
            self.worksheet.write(i+1,2, label = str(self._CurrentCpuUseRate))
            self.worksheet.write(i+1,3, label = str(self._CurrentMemUse))

            # 保存
            self.workbook.save('./Data/PerformanceData.txt.xls')
