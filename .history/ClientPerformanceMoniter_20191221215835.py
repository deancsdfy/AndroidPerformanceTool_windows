...
class ClientPerformanceMoniter(object):
    ...
    def StartMonitor(self):
        ...
        while 1:
            ...
            # 创建一个worksheet
            worksheet = self.workbook.add_sheet('My Worksheet')

            # 写入excel
            # 参数对应 行, 列, 值
            worksheet.write(i+1,1, label = str(time.time()))
            worksheet.write(i+1,2, label = str(self._CurrentCpuUseRate))
            worksheet.write(i+1,3, label = str(self._CurrentMemU
            # 保存
