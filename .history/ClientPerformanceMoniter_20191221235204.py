...
class ClientPerformanceMoniter(object):
    ...
    def StartMonitor(self):
        ...
        while 1:
            ...
            # 写入excel
            # 参数对应 行, 列, 值
            worksheet.write((count+i)%10,0, label = str(now))
            worksheet.write((count+i)%10,1, label = round(float(self._CurrentCpuUseRate),2))
            worksheet.write((count+i)%10,2, label = round(float(self._CurrentMemUse),1))
