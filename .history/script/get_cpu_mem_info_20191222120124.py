...
def getCpuInfo():
    ...
    if(len(top_info)!=0):
        for x in top_info:
            temp_list = x.split()
            # print(temp_list[8])
            if getAndroidVersion == '6.0.1':
                cpu = round(float(str(temp_list[2])[0:-1])/cpunums,2)
            elif (temp_list[8]!=" "):
...
def getAndroidVersion():
    androidVersion = str(util.shell('getprop ro.build.version.sdk').stdout.read())
    print(androidVersion[0:-6])
