...
def top():
    ...
    return (cpu,mem)
def getCpuNums():
    num_info = util.shell('adb shell "cat /proc/cpuinfo|grep processor"').stdout.readlines()
    print(num_info)
...
def getCpuInfo():
    ...
    print(pid)
    cpunums=getCpuNums()
    top_info = util.shell('adb shell "top -n 1 | grep %d"' % (int(pid))).stdout.readlines()
    if(len(top_info)!=0):
        for x in top_info:
            temp_list = x.split()
            # print(temp_list[8])
...
def getMemInfo():
    ...
    pid=get_pid()
    print(pid)
    mem_info = util.shell('adb shell"dumpsys meminfo %d |grep TOTAL:"' %(int(pid))).stdout.readlines()
    for x in mem_info:
        temp_list = x.split()
...
def get_pid():
    ...
    package = util.shell('adb shell "dumpsys activity top| grep ACTIVITY"').stdout.read()
