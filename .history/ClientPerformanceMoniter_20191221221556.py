...
class ClientPerformanceMoniter(object):
    ...
    def StartMonitor(self):
        ...
        while 1:
            ...
            # 创建一个worksheet
            worksheet = workbook.add_sheet('per')

            # 写入excel
            # 参数对应 行, 列, 值
            worksheet.write(count+1,0, label = str(time.time()))
            worksheet.write(count+1,1, label = str(self._CurrentCpuUseRate))
            worksheet.write(count+1,2, label = str(self._CurrentMemUse))
